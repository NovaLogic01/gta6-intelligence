...
export function generateStaticParams() { return features.map((item) => ({ slug: item.slug })) }
export function generateMetadata({ params }: { params: { slug: string } }): Metadata {
  const item = features.find(i => i.slug === params.slug)
  if (!item) return { title: 'Not Found' }
  return { title: `${item.name} — Features`, description: item.description }
}
  ...
  return (
    <div className="section-spacing">
      <div className="container-article">
        <Link href="/database/features" className="inline-flex items-center gap-2 text-body-sm text-text-tertiary hover:text-text-primary transition-colors mb-8">
        ...
        <header className="mb-10 pb-8 border-b border-border-primary">
          <div className="flex flex-wrap items-center gap-3 mb-4">
            ...
            <span className="text-overline text-text-tertiary uppercase tracking-wider">Feature</span>
          </div>
          <h1 className="text-display lg:text-hero text-text-primary font-display mb-3">{feature.name}</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            <section>
              ...
              {feature.knownInformation.length > 0 ? (
                <ul className="space-y-3">
                  {feature.knownInformation.map((info, i) => (
          ...
          <aside className="space-y-4">
            <div className="card-base p-5">
              ...
              <dl className="space-y-3 text-body-sm">
                {feature.featureType && <div><dt className="text-text-muted mb-0.5">Type</dt><dd className="text-text-secondary">{feature.featureType}</dd></div>}
                <div><dt className="text-text-muted mb-0.5">Last Updated</dt><dd className="text-text-secondary">{formatDate(feature.lastUpdated)}</dd></div>
              </dl>
            </div>
            {feature.sourceIds.length > 0 && (
              <div className="card-base p-5">
                <h3 className="text-body-sm font-semibold text-text-primary mb-4 pb-3 border-b border-border-primary">Sources</h3>
                <ul className="space-y-2">
                  {feature.sourceIds.map((sourceId) => { const source = getSourceById(sourceId); if (!source) return null; return (<li key={sourceId}><a href={source.url} target="_blank" rel="noopener noreferrer" className="text-body-sm text-accent-blue hover:text-accent-blue-muted transition-colors">{source.name}</a></li>) })}
                </ul>
              </div>
            )}
          </aside>
